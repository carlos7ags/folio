...
- **`@font-face url('/absolute/path/to/font')` loads when `Options.BaseFS` is nil** — `loadFontFaces` previously routed every filesystem path through `BaseFS` only, so absolute system paths silently failed and the renderer fell through to the system fallback. When `BaseFS` is unset, an absolute path now bypasses asset resolution and loads from the OS directly, mirroring how `Options.FallbackFontPath` already worked. When `BaseFS` is set, a leading `/` continues to resolve at the BaseFS root — programs that need a system font alongside a BaseFS asset tree should pass it via `Options.FallbackFontPath`, which has its own OS-vs-BaseFS resolution (#227)
